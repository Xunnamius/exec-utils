...
import {
  ...
  name as packageName
} from 'rootverse+run:package.json';
...
const debug = createDebugLogger({ namespace: 'run' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+run:package.json'))),
    packageName,
    packageExports
  );
});
